...
export function CoverageComparisonChart() {
  ...
  const radarData = [
    {
      category: "Policies",
      ...Object.fromEntries(top5Payers.map((p) => [p.payer, (p.totalPolicies / maxPolicies) * 100])),
      fill: "#449CFB",
      stroke: "#449CFB",
    },
    {
    ...
    {
      category: "Modifiers",
      ...Object.fromEntries(top5Payers.map((p) => [p.payer, (p.modifiers / maxModifiers) * 100])),
      fill: "#0071EA",
      stroke: "#0071EA",
    },
  ]
  ...
  return (
    <div className="rounded-xl border bg-white p-6 shadow-custom">
      ...
      <p className="mt-4 text-center text-xs text-slate-500">Values shown as percentage of maximum across payers</p>
    </div>
  )
}
